...
!/    Based on W3PREP and modified to allow standard file format input
!/    Natacha Bernier and Jamie McLean
!/
!/                  +-----------------------------------+
...
!     (ww3_shel.ftn).  Assumes Environment Canada standard file input
!
!
!  2. Method :
!
...
!     Local parameters.
!     ----------------------------------------------------------------
!       NDSI    Int.  Input unit number ("ww3_prsf.inp").
...
!      DSEC21    Subr. W3TIMEMD Interval in seconds between time stamps
...
!
!      FSTD_GET_DATA    Subr. W3FSTDMD Read  FST record
!
!     --------------------------------------------------------------------
!
!  5. Called by :
...
!
      USE W3FSTDMD, ONLY: FSTD_GET_DATA
!
      IMPLICIT NONE
!/
!/ ------------------------------------------------------------------- /
...
!EC Standard file parameters
      ...
      character(len=14)            :: IDATEV,IDATEV_END
      integer                      :: IUNIT, IOS
      integer                      :: LEVELS(1:2)
      integer                      :: FCST_HR
      integer                      :: CTR, TIME_END(2)
      real, pointer                :: DATARR(:,:,:,:)
      real, pointer                :: TIME_Interval(:)
      integer                      :: INITTIME(2)
      integer                      :: ndim1,ndim2,ndim3,NRECORDS, NV
      character(len=4), ALLOCATABLE :: INPUT_FIELD(:)
...
!
      WRITE (NDSO,900)
#ifdef W3_S
      CALL STRACE (IENT, 'W3PRSF')
#endif
!
      J      = LEN_TRIM(FNMPRE)
      OPEN (NDSI,FILE=FNMPRE(:J)//'ww3_prsf.inp',STATUS='OLD',        &
            ERR=800,IOSTAT=IERR)
      REWIND (NDSI)
...
!
      ...
      READ (NDSI,*,END=801,ERR=802,IOSTAT=IERR) IDFLD, IDTYPE, FLTIME, &
                                                FLHDR
!      print *,'debug IDFLD, IDTYPE, FLTIME, FLHDR=', &
!               IDFLD,IDTYPE,FLTIME,FLHDR
!
! 3.b Check types.
!
      FLSTAB = IDFLD .EQ. 'WNS'
...
! FLSTAB: Air-sea temperature differences not currently supported in this version
      IF ( FLSTAB ) GOTO 864    ! EXIT with error message
...
! ... time
!
      IF (.not. FLTIME) GOTO 865    ! EXIT with error message
      IF (FLTIME) THEN
          CALL NEXTLN ( COMSTR , NDSI , NDSE )
...
! standard file format
          ...
          WRITE (NDSO,931) IDTIME
          CALL NEXTLN ( COMSTR , NDSI , NDSE )
          READ (NDSI,*,END=801,ERR=802,IOSTAT=IERR) TIME_END
          IF (TIME_END(1).LT.10000000) THEN
              WRITE (NDSE,1035) TIME_END
              CALL EXTCDE ( 4 )
          END IF
        END IF
!
...
!
! Extract format from Standard File
!
! Read input field's standard file variable name
!
        WRITE(*,*) "..Read input field's standard file variable name"
        CALL NEXTLN ( COMSTR , NDSI , NDSE )
        READ (NDSI,*,END=801,ERR=802,IOSTAT=IERR) NOMVAR
        WRITE(*,*) "NOMVAR=",NOMVAR
        WRITE (NDSO,968) NOMVAR
!
! Read input field's forecast level
!
        WRITE(*,*) "..Read input field's forecast level"
        CALL NEXTLN ( COMSTR , NDSI , NDSE )
        READ (NDSI,*,END=801,ERR=802,IOSTAT=IERR) LEVELS(1), LEVELS(2)
        WRITE(*,*) "LEVELS=",LEVELS(1), LEVELS(2)
        !WRITE (NDSO,969) LEVELS

! Read input field's units
!
        WRITE(*,*) "..Read input field's units"
        CALL NEXTLN ( COMSTR , NDSI , NDSE )
        READ (NDSI,*,END=801,ERR=802,IOSTAT=IERR) UNITS
        WRITE(*,*) "UNITS=", UNITS
        !WRITE (NDSO,969) UNITS

        IF (FROMF.EQ.'NAME') WRITE (NDSO,967) NAMEF

!--- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
! 5   Prepare files
! 5.a Open input Standard File
!
!
        CTR=1

        WRITE (NDSO,970)
        WRITE(*,*) "Getting FSTD data"

!Careful -- multiple input files not allowed yet

        ! One file - to be prepared before calling ww3_prsf
        FILENAME=NAMEF
        WRITE(*,*) "filename=",filename
        IUNIT=11
        TYPE=FSTD_TYPE_RO
        WRITE(IDATEV,'(I8,I6.6)') TIME(1), TIME(2)
        WRITE(IDATEV_END,'(I8,I6.6)') TIME_END(1), TIME_END(2)
        WRITE(*,*) "IDATEV=", IDATEV
        WRITE(*,*) "IDATEV_END=", IDATEV_END

        ! If fstd nomvar is 2D field then set the corresponding nomvar for
        ! each dimension
        IF (NOMVAR == "UV") THEN
           ALLOCATE ( INPUT_FIELD(2) )
           INPUT_FIELD(1)="UU"
           INPUT_FIELD(2)="VV"
        ELSEIF (NOMVAR == "UV2W") THEN
           ALLOCATE ( INPUT_FIELD(2) )
           INPUT_FIELD(1)="UU2W"
           INPUT_FIELD(2)="VV2W"
        ELSE
           ALLOCATE ( INPUT_FIELD(1) )
           INPUT_FIELD(1)=NOMVAR
        END IF

        WRITE(*,*) "NBB", IFLD, NOMVAR
        DO NV=1,SIZE(INPUT_FIELD)
           WRITE(*,*) "INPUT_FIELD(", NV, ") ", INPUT_FIELD(NV)
        END DO

        ! Set the variable type -- Carefull assumed type
        TYPVAR='P@'

        WRITE(*,*) "BEFORE FSTD_GET_DATA"
        ! One file - pre-processed with fields mapped on output grid prior to invoking ww3_prsf
        CALL FSTD_GET_DATA(FILENAME,IUNIT,TYPE,TYPVAR,IDATEV,IDATEV_END,LEVELS,UNITS,&
                           INPUT_FIELD,DATARR,IOS,NRECORDS,TIME_Interval,INITTIME)
!       WRITE(*,*) "NBB call to fstgetdata", DATARR(1:720,1:43,1:3,1)
        WRITE(*,*) "AFTER FSTD_GET_DATA"
...
! ... Input
!
! Strait read from fst file

         IF ( ITYPE .LE. 4 ) THEN
               WRITE(*,*) "ITYPE=",ITYPE
            IF (IFLD.LT.3) THEN
               XC=DATARR(:,:,FCST_HR,1)
            ELSE IF (IFLD.GE.3 .OR. FLBERG) THEN
               XC=DATARR(:,:,FCST_HR,1)
               YC=DATARR(:,:,FCST_HR,2)
! FLSTAB NOT CURRENTLY SUPPORTED
!               FA=DATARR(:,:,FCST_HR,3)
            END IF
          ELSE
         ...
         ELSE IF ( ITYPE .EQ. 5 ) THEN
             IF ( NDAT .EQ. 0 ) THEN
              ...
              ELSE
                ...
                WRITE (NDSO,977)
#endif
              WRITE(*,*) "START2-Write Fields"
                CALL W3FLDD ('WRITE', IDFLD, NDSDAT, NDST, NDSE, TIME,&
                             TIME, RECLDT, NDAT, IDAT, DATA, IERR )
               WRITE(*,*) "END2-Write Fields"
                DEALLOCATE ( DATA )
              END IF
         END IF
         IF (IERR.NE.0) CALL EXTCDE ( 30 )
         IF (FCST_HR .LT. NRECORDS) THEN    ! last time interval does not exist
            WRITE(*,*) "TIME-Before Incrementing=",TIME
            CALL TICK21(TIME, TIME_Interval(FCST_HR)*3600.)
            WRITE(*,*) "TIME-After Incrementing=",TIME
         ENDIF
...
!
  ...
  968 FORMAT ( '           Standard File variable name: ',A)
!
...
!
 1000 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     ERROR IN OPENING INPUT FILE'/                    &
               '     IOSTAT =',I5/)
!
 1001 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     PREMATURE END OF INPUT FILE'/)
!
 1002 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     ERROR IN READING FROM INPUT FILE'/               &
               '     IOSTAT =',I5/)
!
 1030 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     ILLEGAL FIELD ID -->',A,'<--'/)
 1031 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     ILLEGAL FORMAT ID -->',A,'<--'/)
 1032 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     THIS FORMAT TYPE IS ALLOWED FOR ICE AND LEV ONLY'/)
!
 1033 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     ILLEGAL DATA RECORD LENGTH : ',I6/)
 1034 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     ILLEGAL DATA TYPE : ',I2/)
!
 1035 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     ILLEGAL TIME : ',I8.8,I7.6/)
 1036 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     ILLEGAL SIZE OF INPUT GRID : ',I5,1X,I5/)
 1037 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     ATTEMPT TO USE INPUT UNIT (',I2,                 &
               ') FOR NAMED FILE'/)
 1038 FORMAT (/' *** WAVEWATCH III WARNING IN W3PRSF : '/             &
               '     DATA READ FROM INPUT FILE')
 1039 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     ARRAYS FOR INPUT FIELDS TO SMALL '/              &
               '            GRID SIZE       :',2I5/                   &
               '            ARRAY DIMENSION :',2I5/)
!
 1040 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     LONGITUDE GRID RANGE > 360 DEGR. : ',2F6.1/)
 1041 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     ILLEGAL LATITUDE GRID RANGE : ',2F6.1/)
 1042 FORMAT (/' *** WAVEWATCH-III WARNING W3PRSF : '/                &
               '     GRID POINT ',2I6,2F7.2,/                         &
               ' NOT COVERED BY INPUT GRID.'/)
 1043 FORMAT (/' *** WAVEWATCH III WARNING W3PRSF : '/                &
               '     X = ',F10.1,' NOT COVERED BY INPUT GRID.'/)
 1044 FORMAT (/' *** WAVEWATCH III WARNING W3PRSF : '/                &
               '     Y = ',F10.1,' NOT COVERED BY INPUT GRID.'/)
!

!
 1045 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     ERROR IN OPENING LAT-LONG DATA FILE'/            &
               '     IOSTAT =',I5/)
!
 1046 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     ERROR IN OPENING MASK FILE'/                     &
               '     IOSTAT =',I5/)
!
 1050 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     ERROR IN OPENING INPUT DATA FILE'/               &
               '     IOSTAT =',I5/                                    &
               '     NDSF   =',I5/                                    &
               '     NAMEF  = ',A/)
 1051 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     CANNOT READ UNFORMATTED FROM UNIT',I3/)
!
 1052 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               ...
               '                  ARRAY DIMENSION = ',2I5/)
!
 1060 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     ERROR IN READING TIME FROM FILE (',I1,')'/       &
               '     IOSTAT =',I5/)
 1061 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     INCOMPATIBLE FIELD TIMES '/                      &
               '            FIELD #1 : ',I8.8,I7.6/                   &
               '            FIELD #2 : ',I8.8,I7.6/)
 1062 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     ERROR IN READING NDAT FROM FILE'/                &
               '     IOSTAT =',I5/)
 1063 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     ERROR IN READING DATA RECORD',I6,' FROM FILE'/   &
               '     IOSTAT =',I5/)
 1064 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     Air-sea temperature differences (FLSTAB) not'/   &
               '     currently supported in this version '/)
 1065 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/               &
               '     FLTIME set to false in ww3_prsf.inp' /           &
               '     is not compatible with STDF input mode' /)

#ifdef W3_O15
 1070 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/          &
               '     ERROR IN CREATING A TIMES FILE FOR ',A/     &
               '     IOSTAT =',I5/)
 1071 FORMAT (/' *** WAVEWATCH III ERROR IN W3PRSF : '/          &
               '     ERROR IN WRITING TIME OUTPUT ',A/           &
               '     IOSTAT =',I5/)
#endif
!
#ifdef W3_T
 9000 FORMAT (' TEST W3PRSF : ACC : ',F6.3)
!
 9040 FORMAT (' TEST W3PRSF : INPUT GRID RANGES AND INCR. AFTER CORR.'/ &
              '               LON / X : ',3F10.2,                   &
              ' (GLOBAL=',L1,')'/                                     &
              '               LAT / Y : ',3F10.2)
 9041 FORMAT (' TEST W3PRSF : INTERPOLATION DATA FOR ',A)
 9042 FORMAT ('              ',I4,F8.2,2I4,2F8.2,1X,F6.3,1X,A)
 9043 FORMAT (' TEST W3PRSF : GRID SHIFTED BY ',F5.0,' DEGREES / M')
#endif
#ifdef W3_T1
 9045 FORMAT (' TEST W3PRSF : IX, IY, IXI(2), IYI(2), RD(4)')
 9046 FORMAT ('     ',2I4,2X,4I4,2X,4F6.2)
#endif
!
#ifdef W3_T1a
 9050 FORMAT (' TEST W3PRSF : LAT-LONG OF INPUT FILE ')
 9051 FORMAT ('             ',2I4,2F8.2,I4)
#endif
!
#ifdef W3_T2
 9060 FORMAT (' TEST W3PRSF : INPUT FIELD (',I1,') :'/)
 9061 FORMAT (' TEST W3PRSF : INPUT DATA RECORDS :')
 9062 FORMAT ('      ',I6,' : ',6E11.3)
 9063 FORMAT ('               ',6E11.3)
#endif
#ifdef W3_T3
 9065 FORMAT (' TEST W3PRSF : OUTPUT FIELD(S) :'/)
#endif
!/
!/ End of W3PRSF ----------------------------------------------------- /
!/
